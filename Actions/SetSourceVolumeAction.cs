...
namespace SuchByte.OBSWebSocketPlugin.Actions
{
    public class SetSourceVolumeAction : ActionBase
    {
        ...
        public override string Description =>
            PluginLanguageManager.PluginStrings.ActionSetSourceVolumeDescription;
        ...
        public override void Trigger(string clientId, ActionButton actionButton)
        {
            if (!String.IsNullOrWhiteSpace(this.Configuration))
            {
                try
                {
                    ...
                    {
                        int currentDecibel = (
                            await conn.OBS.InputsRequests.GetInputVolumeAsync(sourceName)
                        ).InputVolumeDb; // fallback if parse failed
                        switch (config.Method)
                        {
                            case Enum.IncrementalMethodType.Set:
                                await conn.OBS.InputsRequests.SetInputVolumeAsync(
                                    sourceName,
                                    inputVolumeDb: config.Decibel
                                );
                                break;
                            case Enum.IncrementalMethodType.Increase:
                                await conn.OBS.InputsRequests.SetInputVolumeAsync(
                                    sourceName,
                                    inputVolumeDb: currentDecibel + config.Decibel
                                );
                                break;
