...
namespace SuchByte.OBSWebSocketPlugin.Actions
{
    public class SourceVisibilityAction : ActionBase
    {
        ...
        public override string Description =>
            PluginLanguageManager.PluginStrings.ActionSourceVisibilityDescription;
            ...
            if (!String.IsNullOrWhiteSpace(this.Configuration))
            {
                try
                {
                    ...
                    _ = Task.Run(async () =>
                    {
                        var sceneItemId =
                            (
                                await conn.OBS.SceneItemsRequests.GetSceneItemIdAsync(
                                    sceneName,
                                    sourceName
                                )
                            )?.SceneItemId ?? -1;
                        if (sceneItemId == -1)
                            return;
                        var sceneItemEnabled = (
                            await conn.OBS.SceneItemsRequests.GetSceneItemEnabledAsync(
                                sceneName,
                                sceneItemId
                            )
                        )?.SceneItemEnabled;
                        ...
                        switch (config.Method)
                        {
                            case Enum.VisibilityMethodType.Hide:
                                await conn.OBS.SceneItemsRequests.SetSceneItemEnabledAsync(
                                    sceneName,
                                    sceneItemId,
                                    false
                                );
