...
namespace SuchByte.OBSWebSocketPlugin.Actions
{
    public class SetAudioMutedAction : ActionBase
    {
        ...
        public override string Description =>
            PluginLanguageManager.PluginStrings.ActionSetAudioMutedDescription;
                ...
                try
                {
                    ...
                    switch (config.Method)
                    {
                        ...
                        case AudioMethodType.Toggle:
                            _ = Task.Run(async () =>
                            {
                                var mutedState = await conn.OBS.InputsRequests.GetInputMuteAsync(
                                    sourceName
                                );
                                await conn.OBS.InputsRequests.SetInputMuteAsync(
                                    sourceName,
                                    !mutedState.InputMuted
                                );
                            });
                            break;
                    }
