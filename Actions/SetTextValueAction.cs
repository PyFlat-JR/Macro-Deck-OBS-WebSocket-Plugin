...
namespace SuchByte.OBSWebSocketPlugin.Actions
{
    public class SetTextValueAction : ActionBase
    {
        ...
        public override string Description =>
            PluginLanguageManager.PluginStrings.ActionSetTextValueDescription;
        ...
        public override void Trigger(string clientId, ActionButton actionButton)
        {
            if (!String.IsNullOrWhiteSpace(this.Configuration))
            {
                try
                {
                    ...
                    _ = Task.Run(async () =>
                    {
                        var response = await conn.OBS.InputsRequests.GetInputSettingsAsync(
                            sourceName
                        );
                        var inputSettings = JObject.FromObject(response.InputSettings);
                        inputSettings["text"] = TemplateManager.RenderTemplate(value);

                        await conn.OBS.InputsRequests.SetInputSettingsAsync(
                            sourceName,
                            inputSettings
                        );
                    });
